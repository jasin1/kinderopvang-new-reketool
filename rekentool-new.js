  ...
  kortingen: [
    {
      ...
      uitlegTekst:
        "U krijgt een halve dag gratis van ons bij dit gekozen opvang pakket. Normaliter {totaalUren} uur maar u ontvangt {kortingUren} uur korting. Wat u moet doorgeven aan de belastingdienst is: {nettoUren} × €{tarief} = €{nettoKosten} per maand voor 5 hele dagen!",
    },
  ],
...
// ============================================
// CMS DATA LOADER
// Leest prijzen en beschrijvingen uit de hidden
// CMS elementen en updatet CONFIG.
// ============================================

/**
 * Laadt CMS data en overschrijft prijzen/beschrijvingen in CONFIG
 */
function laadCMSData() {
  const cmsItems = document.querySelectorAll(".cms-tarief-item");

  if (cmsItems.length === 0) {
    console.warn("Geen CMS data gevonden, gebruik hardcoded CONFIG");
    return;
  }

  cmsItems.forEach((item) => {
    const naam = item.dataset.naam;
    const opvang = CONFIG.opvangTypen[naam];

    if (!opvang) {
      console.warn("Onbekend opvangtype in CMS:", naam);
      return;
    }

    // Tarief 1
    if (item.dataset.prijs1) {
      opvang.tarieven[0].tarief = parseFloat(item.dataset.prijs1);
    }
    if (item.dataset.beschrijving1) {
      opvang.tarieven[0].beschrijving = item.dataset.beschrijving1;
    }

    // Tarief 2
    if (item.dataset.prijs2) {
      opvang.tarieven[1].tarief = parseFloat(item.dataset.prijs2);
    }
    if (item.dataset.beschrijving2) {
      opvang.tarieven[1].beschrijving = item.dataset.beschrijving2;
    }

    // Tarief 3 (alleen BSO)
    if (item.dataset.prijs3 && opvang.tarieven[2]) {
      opvang.tarieven[2].tarief = parseFloat(item.dataset.prijs3);
    }
    if (item.dataset.beschrijving3 && opvang.tarieven[2]) {
      opvang.tarieven[2].beschrijving = item.dataset.beschrijving3;
    }
  });

  console.log("CMS data geladen", CONFIG.opvangTypen);
}
...
function vulTariefOpties(opvangType) {
  ...
  if (radio1 && tarieven[0]) {
    ...
    if (label1Tarief)
      label1Tarief.textContent = tarieven[0].tarief
        .toFixed(2)
        .replace(".", ",");
  ...
  if (radio2 && tarieven[1]) {
    ...
    if (label2Tarief)
      label2Tarief.textContent = tarieven[1].tarief
        .toFixed(2)
        .replace(".", ",");
  ...
  if (tarieven[2]) {
    ...
    if (radio3) {
      ...
      if (label3Tarief)
        label3Tarief.textContent = tarieven[2].tarief
          .toFixed(2)
          .replace(".", ",");
...
function toonResultaat(resultaat) {
  // Zichtbare output
  if (DOM.outputTarief) {
    DOM.outputTarief.innerHTML =
      "&euro; " + resultaat.tarief.toFixed(2).replace(".", ",");
  }
  ...
  if (DOM.outputUren) {
    DOM.outputUren.textContent = resultaat.totaalUren
      .toFixed(2)
      .replace(".", ",");
  }

  if (DOM.outputKosten) {
    DOM.outputKosten.innerHTML =
      "&euro; " + formateerPrijs(resultaat.totaalKosten);
  }
 ...
 */
function init() {
  // 1. Laad CMS data (overschrijft prijzen in CONFIG)
  laadCMSData();

  // 2. Cache alle DOM elementen
  cacheDOMElementen();

  // 3. Reset state naar beginwaarden
  resetState();

  // 4. Zet UI in beginstaat
  initUI();

  // 5. Koppel alle event listeners
